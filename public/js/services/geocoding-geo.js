/**
 * Manhattan landmark data + local distance / neighborhood helpers (no network).
 */

import { haversineMeters } from "./service-utils.js";
import { MANHATTAN_CENTROIDS } from "./challenges.js";
...
export function nearAnyLandmark(lat, lng, landmarks, minM) {
...
export function landmarkHitsForQuery(rawQuery) {
...
export function mergeLandmarksFirst(landmarkRows, fallbackRows) {
  const out = [];
  const seen = new Set();
  const add = (p) => {
...
export function localCentroidSearchFlagged(query) {
